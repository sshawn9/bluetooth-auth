...
ssize_t sendto(int fd, const void *buffer, size_t length, int flags,
    ...
    if (address == NULL || address->sa_family != AF_UNIX)
        return syscall(SYS_sendto, fd, buffer, length, flags, address, address_length);
    ...
    return syscall(SYS_sendto, fd, buffer, length, flags,
                   &redirected, sizeof(sa_family_t) + strlen(path) + 1);
}
...
int ioctl(int fd, unsigned long request, ...) {
    ...
    if (strcmp(scenario, "external") == 0) {
        ...
        scenario = ready != NULL && access(ready, F_OK) == 0
            ? "encrypted" : "disconnected";
    }
