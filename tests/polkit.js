...
check(evaluate(trusted, "test.bluetooth-action") === "yes", "eligible action was not authorized");
check(spawnCalls.length === 1, "eligible action did not invoke helper");
check(spawnCalls[0].length === 5 && spawnCalls[0][1] === "--address-file" &&
      spawnCalls[0][2] === "/private/test-address" && spawnCalls[0][3] === "--connect" &&
      spawnCalls[0][4] === "-1",
      "helper arguments differ from the connect-notification contract");
check(evaluate(trusted, "test.bluetooth-action", true) === "not-handled", "helper error bypassed fallback");
...
for (const [subject, action] of [
  ...
  check(evaluate(subject, action) === "not-handled", "ineligible request was handled");
