...
class ReleaseTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_disconnect_cannot_be_hidden_by_reusing_the_handle(self):
        ...
        release.check_continuity(
            7, [link()], PHONE, [{"address": PHONE, "address_type": 0}]
        )
    ...
    def test_target_requires_exact_unblocked_record_not_authentication_policy(self):
        ...
        for field in ("Paired", "Bonded", "Trusted"):
            with self.subTest(field=field):
                self.assertEqual(
                    release.target(
                        {"/target": {release.DEVICE: {**phone, field: False}}},
                        adapter,
                        PHONE,
                    )[0],
                    "/target",
        ...
        self.assertEqual(
            release.target(
                {"/target": {release.DEVICE: missing_bonded}}, adapter, PHONE
            )[0],
            "/target",
        )
        with self.assertRaisesRegex(release.Inconclusive, "Blocked=true"):
            release.target(
                {"/target": {release.DEVICE: {**phone, "Blocked": True}}},
                adapter,
                PHONE,
            )
    ...
    async def test_provider_closes_only_its_bus_without_disconnect_or_settings_calls(
        self,
    ):
        backend = SimpleNamespace(
            bus=SimpleNamespace(
                unique_name=":1.77", wait_for_disconnect=mock.AsyncMock()
            ),
            open=mock.AsyncMock(),
            close=mock.AsyncMock(),
            register=mock.AsyncMock(),
            objects=mock.AsyncMock(
                return_value={
                    "/org/bluez/hci0": {
                        release.ADAPTER: {"Alias": "Computer", "UUIDs": []}
                    },
                    "/target": {
                        release.DEVICE: {
                            "Address": PHONE,
        ...
        with (
            mock.patch.object(release, "Backend", return_value=backend),
            ...
            mock.patch.object(
                release.asyncio, "to_thread", new=mock.AsyncMock(return_value=b"")
            ),
            contextlib.redirect_stdout(io.StringIO()),
        ):
            await release.provider(SimpleNamespace(adapter="hci0"), PHONE)
        self.assertEqual(
            backend.register.await_args_list,
            [
    ...
    async def test_provider_registration_error_also_closes_bus(self):
        ...
        backend = SimpleNamespace(
            bus=None, open=mock.AsyncMock(side_effect=error), close=mock.AsyncMock()
        )
        output = io.StringIO()
        with (
            mock.patch.object(release, "Backend", return_value=backend),
            ...
            self.assertEqual(
                await release.provider(SimpleNamespace(adapter="hci0"), PHONE), 2
            )
        backend.close.assert_awaited_once()
    ...
    async def test_provider_reports_close_error_without_leaking_exception_text(self):
        backend = SimpleNamespace(
            bus=None,
            open=mock.AsyncMock(side_effect=RuntimeError("fixture")),
    ...
    async def simulate(
        ...
        *,
        ...
        preexisting=False,
        disconnect_on_register=False,
        replace_on_register=False,
        lose_companion=False,
        phone_properties=None,
        ...
        def active():
            return (
                connected
                and (preexisting or world["started"])
        ...
        async def objects():
            ...
            if premature_exit and world["clock"] >= 2:
                close_child()
            registered = (
                world["started"] and not world["exited"] and not registration_error
            )
            return {
                adapter_path: {
                    release.ADAPTER: {
                    ...
                    release.GATT_MANAGER: {},
                    release.AD_MANAGER: {"ActiveInstances": int(registered)},
                },
                "/target": {
                    release.DEVICE: {
                        "Address": PHONE,
        ...
        mgmt = SimpleNamespace(
            close=mock.AsyncMock(),
            disconnect_events=disconnected,
        ...
        reader = SimpleNamespace(
            read=lambda _: (
                [
                    SimpleNamespace(
                        address=PHONE,
                        link_type=release.HCI_LE_LINK,
                        state=release.BT_CONNECTED,
        ...
        with (
            mock.patch.object(release, "Backend", return_value=backend),
            mock.patch.object(
                release, "open_link", new=mock.AsyncMock(return_value=mgmt)
            ),
            mock.patch.object(release, "LinkReader", return_value=reader),
            mock.patch.object(
                release.asyncio, "create_subprocess_exec", side_effect=spawn
            ),
            mock.patch.object(release.asyncio, "sleep", side_effect=sleep),
            mock.patch.object(
                release, "time", SimpleNamespace(monotonic=lambda: world["clock"])
            ),
            contextlib.redirect_stdout(output),
        ):
            code = await release.experiment(
                SimpleNamespace(
                    phone_file="fixture-address",
                    adapter="hci0",
                    wait_seconds=0.02,
                    observe_seconds=1,
    ...
    async def test_cached_hid_without_fresh_read_can_enter_release_and_pass(self):
        ...
        self.assertLess(
            names.index("advertising"), names.index("waiting_for_connection")
        )
    ...
    async def test_manual_setup_is_labelled_without_claiming_automatic_reconnection(
        self,
    ):
        ...
        for name in ("waiting_for_connection", "connected", "result"):
            self.assertEqual(
                next(event for event in events if event["event"] == name)[
                    "connection_setup"
                ],
                "manual",
            )
    ...
    async def test_blocked_target_reports_the_actual_property_before_starting_provider(
        self,
    ):
        code, events = await self.simulate(
            preexisting=True, phone_properties={"Blocked": True}
        )
        self.assertEqual(code, 2)
        result = next(event for event in events if event["event"] == "result")
    ...
    async def test_existing_target_loss_after_release_is_failed_not_companion_interference(
        self,
    ):
    ...
    async def test_other_original_connections_remain_protected_when_reusing_target(
        self,
    ):
    ...
    async def test_provider_exit_during_baseline_is_not_a_successful_planned_release(
        self,
    ):
    ...
    async def test_release_errors_are_reported_without_reusing_the_old_link_snapshot(
        self,
    ):
        cases = [
            {"exit_code": 2},
        ...
        for case in cases:
            with self.subTest(case=case):
                ...
                if any(
                    item.get("event") in {"provider_error", "error"}
                    for item in case.get("shutdown_events", [])
                ):
                    self.assertIn(
                        "provider_error", [event["event"] for event in events]
                    )
