...
class FakeSocket:
    def __init__(self):
        self.closed, self.recv_value, self.recv_error = False, None, None
...
class CoexistTraceTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.monitor = trace.CoexistTrace(
            self.socket, 1, lambda name, data: self.events.append((name, data))
        )

    def feed_command(self, opcode, parameters, index=1):
        self.monitor._receive(
            record(trace.HCI_MON_COMMAND_PKT, index, command(opcode, parameters))
        )

    def test_legacy_parameters_and_safe_ad_summary(self):
        self.feed_command(
            0x2006,
            struct.pack("<HH", 0x20, 0x40)
            + bytes((0, 1, 0))
            + b"secret"
            + bytes((7, 2)),
        )
    ...
    def test_extended_commands_and_whitelisted_status(self):
        ...
        self.monitor._receive(
            record(trace.HCI_MON_EVENT_PKT, 1, bytes((0x0E, len(complete))) + complete)
        )
        ...
        self.assertEqual(
            self.events[1][1]["sets"],
            [{"handle": 3, "duration_ms": 20, "max_events": 4}],
        )
        self.assertEqual(
            self.events[2],
            (
                "advertising_status",
                {"opcode": "0x2036", "status": 0, "event": "complete"},
            ),
        )
    ...
    def test_unknown_acl_ltk_and_malformed_data_never_emit_or_leak(self):
        ...
        self.feed_command(
            0x2019, secret
        )  # LE Start Encryption, deliberately not allowed
        self.monitor._receive(
            record(trace.HCI_MON_EVENT_PKT, 1, bytes((0x3E, 3, 1, 2, 3)))
        )
            ...
            [
                (
                    "advertising_command",
                    {
                        "opcode": "0x2035",
                        "handle": 7,
                        "configured_random_address": True,
                    },
                )
            ],
        )
    ...
    async def test_index_filter_status_and_closed_handles(self):
        ...
        self.monitor._receive(
            record(trace.HCI_MON_EVENT_PKT, 1, bytes((0x0F, len(status))) + status)
        )
        self.assertEqual(
            self.events,
            [
                (
                    "advertising_status",
                    {"opcode": "0x200a", "status": 12, "event": "status"},
                )
            ],
        )
