...
class FakeBus:
    def __init__(self, fail_on=None):
        self.exports, self.fail_on = {}, fail_on
...
class CoexistGattTests(unittest.TestCase):
    def setUp(self):
        ...
        self.app = gatt.HidApplication(
            self.bus,
            "/com/example/hid",
            self.emit,
            lambda path: path == "/org/bluez/hci0/dev_OK",
        )
        self.options = {
            "device": Variant("o", "/org/bluez/hci0/dev_OK"),
            "link": Variant("s", "le"),
        }
    ...
    def test_exports_complete_hogp_object_manager_with_correct_signatures(self):
        ...
        self.assertEqual(
            self.app.report_reference.read_value(self.options), b"\x01\x01"
        )
    ...
    def test_read_offsets_authorization_and_target_events(self):
        self.assertEqual(
            self.app.report.read_value({**self.options, "offset": Variant("q", 1)}), b""
        )
        with self.assertRaises(DBusError) as denied:
            self.app.report_map.read_value(
                {"device": Variant("o", "/org/bluez/hci0/dev_NO")}
            )
    ...
    def test_export_failure_unexports_every_successful_item(self):
        failing_bus = FakeBus(
            ("/com/example/hid/service0/char1", gatt.GATT_CHARACTERISTIC)
        )
        app = gatt.HidApplication(
            failing_bus, "/com/example/hid", self.emit, lambda _path: True
        )
    ...
    def test_never_sends_report_and_subscription_is_global(self):
        ...
        self.assertEqual(
            self.events[-1],
            ("gatt_subscription", {"attribute": "report", "scope": "global"}),
        )
