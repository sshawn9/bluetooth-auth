...
class ObserveTests(unittest.IsolatedAsyncioTestCase):
        ...
        *,
        ...
        def active():
            return connected and (
                survive or reconnect_after_disconnect or world["time"] < 0.4
            )

        async def objects():
            return {
                adapter: {
                    observe.ADAPTER: {"UUIDs": ["1812"] if hid else []},
        ...
        async def links():
            return (
                ...
                if active()
                and mgmt_connected
                and not (mgmt_drops and world["time"] >= 0.4)
                else set()
            )
        ...
        def read(_):
            ...
            return [
                SimpleNamespace(
                    address=PHONE,
                    link_type=observe.HCI_LE_LINK,
                    state=observe.BT_CONNECTED,
                    handle=7,
                    encrypted=encrypted,
                )
            ]
        ...
        async def sleep(seconds):
            world["time"] += seconds
            if (
                not active() or reconnect_after_disconnect and world["time"] >= 0.4
            ) and not disconnects:
                disconnects.append({"address": PHONE, "address_type": 2})
            await real_sleep(0)

        backend = SimpleNamespace(
            open=mock.AsyncMock(), close=mock.AsyncMock(), objects=objects
        )
        mgmt = SimpleNamespace(
            close=mock.AsyncMock(), connections=links, disconnect_events=disconnects
        )
        with (
            mock.patch.object(observe, "Backend", return_value=backend),
            mock.patch.object(
                observe, "open_link", new=mock.AsyncMock(return_value=mgmt)
            ),
            mock.patch.object(
                observe, "LinkReader", return_value=SimpleNamespace(read=read)
            ),
            mock.patch.object(observe.asyncio, "sleep", side_effect=sleep),
            mock.patch.object(
                observe, "time", SimpleNamespace(monotonic=lambda: world["time"])
            ),
            contextlib.redirect_stdout(output),
        ):
            code = await observe.observe(
                SimpleNamespace(adapter="hci0", wait_seconds=0.2, observe_seconds=1),
                PHONE,
            )
    ...
    async def test_encrypted_link_passes_without_hid(self):
        ...
        self.assertEqual(
            next(item for item in events if item["event"] == "result")["verdict"],
            "passed",
        )
    ...
    async def test_hid_or_advertisement_is_inconclusive(self):
        for kwargs in ({"hid": True}, {"advertisements": 1}):
            with self.subTest(kwargs=kwargs):
                ...
                self.assertEqual(
                    next(item for item in events if item["event"] == "result")[
                        "verdict"
                    ],
                    "inconclusive",
                )
    ...
    def test_missing_resource_metadata_is_inconclusive(self):
        for objects in (
            {},
            {"/org/bluez/hci0": {observe.ADAPTER: {"UUIDs": []}}},
            {
                "/org/bluez/hci0": {
                    observe.ADAPTER: {},
                    observe.AD_MANAGER: {"ActiveInstances": 0},
                }
            },
            {
                "/org/bluez/hci0": {
                    observe.ADAPTER: {"UUIDs": []},
                    observe.AD_MANAGER: {},
                }
            },
        ):
    ...
    async def test_unencrypted_link_times_out(self):
        ...
        self.assertEqual(code, 2)
        self.assertEqual(
            next(item for item in events if item["event"] == "result")["phase"],
            "connect",
        )
    ...
    async def test_mgmt_loss_during_observation_cannot_pass_with_hci_still_present(
        self,
    ):
