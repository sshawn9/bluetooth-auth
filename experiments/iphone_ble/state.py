...
class LabState:
    ...
    def validate(self) -> dict:
        ...
        for mode, identity in manifest["modes"].items():
            ...
            if identity.get("name") != MODES[mode] or not re.fullmatch(
                r"(?:[0-9A-F]{2}:){5}[0-9A-F]{2}", address
            ):
            ...
            if (
                not re.fullmatch(r"[0-9a-f]{32}", irk)
    ...
    def prepare(self) -> dict:
        ...
        for mode, name in MODES.items():
            while True:
                ...
                if address not in addresses and (
                    int.from_bytes(raw) & ((1 << 46) - 1)
                ) not in (0, (1 << 46) - 1):
                    break
            addresses.add(address)
            manifest["modes"][mode] = {
        ...
        for mode, identity in manifest["modes"].items():
            ...
            if config_path.exists():
                if read_json(config_path) != expected:
                    raise ValueError(
                        f"配置被修改：{config_path}；不会覆盖或重新生成身份"
                    )
            else:
    ...
    def public_manifest(self) -> dict:
        manifest = self.validate()
        return {
            mode: {key: identity[key] for key in ("name", "address")}
            for mode, identity in manifest["modes"].items()
        }
    ...
    def has_bond(self, mode: str) -> bool:
        ...
        return any(
            ...
            and any(
                ...
                and any(
                    isinstance(keys.get(name), dict) and keys[name].get("value")
                    for name in ("ltk", "ltk_central", "ltk_peripheral")
                )
                for keys in peers.values()
            )
            for peers in data.values()
        )
    ...
    def append_event(self, event: str, payload: dict) -> None:
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as stream:
            stream.write(
                json.dumps(
                    {"time": time.time(), "event": event, **payload}, ensure_ascii=False
                )
                + "\n"
            )
            stream.flush()
    ...
    def purge(self) -> list[str]:
        ...
        with self.lock():
            if (
                self.journal.exists()
                or self.journal.with_name(self.journal.name + ".tmp").exists()
            ):
                raise RuntimeError(
                    "存在未完成的适配器恢复记录；先运行 restore，禁止删除恢复依据"
                )
            allowed = {
                Path(name)
                for name in (
            ...
            for path in entries:
                ...
                if not (
                    path.is_file() or (relative in map(Path, MODES) and path.is_dir())
                ):
