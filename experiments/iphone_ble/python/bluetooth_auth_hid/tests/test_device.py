...
class FakeBus:
    def __init__(
        ...
        *,
        fail_member=None,
        release_on_advertisement=False,
        wait_error=None,
        unexport_error=None,
    ):
    ...
    async def call(self, message):
        ...
        if message.member == "RegisterAdvertisement":
            if self.advertisement_gate is not None:
                await self.advertisement_gate.wait()
            if message.member != self.fail_member:
                self.advertising = True
        if message.member == "RegisterAdvertisement" and self.release_on_advertisement:
            self.advertising = False
            next(
                item for path, item in self.exports if path == device.ADVERTISEMENT
            ).Release()
        if message.member == self.fail_member:
            return Message(
        ...
        if message.member == "UnregisterAdvertisement":
            ...
            if self.unregister_gate is not None:
                await self.unregister_gate.wait()
    ...
    def export(self, path, item):
        self.exports.append((path, item))
...
class DeviceTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.patches = [
            patch.object(device, "MessageBus", side_effect=lambda **_kwargs: self.bus)
        ]
    ...
    def context(self):
        return device.BluetoothContext(ADDRESS)
    ...
    async def test_register_is_idempotent_concurrent_and_unregister_closes_once(self):
        ...
        self.assertEqual(
            await asyncio.gather(*(device.register_hid(ctx) for _ in range(3))),
            [True] * 3,
        )
        ...
        self.assertEqual(
            [
                item.member
                for item in self.bus.calls
                if item.member.startswith("Register")
            ],
            ["RegisterApplication"],
        )
        ...
        self.assertFalse(
            ...
            & {item.member for item in self.bus.calls}
        )
    ...
    async def test_partial_failure_closes_and_allows_later_registration(self):
        ...
        with contextlib.redirect_stderr(output):
            self.assertFalse(await device.register_hid(ctx))
        self.assertEqual(len(output.getvalue().splitlines()), 1)
        self.assertTrue(self.bus.closed)
        self.assertEqual(self.bus.cleanup_lock_states, [True])
        self.assertEqual(
            (lock.acquire_count, lock.release_count, lock.locked()), (1, 1, False)
        )
        self.assertIsNone(ctx._bus)
        self.bus = FakeBus()
    ...
    async def test_release_during_connect_does_not_unregister_hid(self):
        ...
        with (
            patch.object(device, "_query", new=Mock(return_value=False)),
    ...
    async def test_query_requires_exact_one_connected_encrypted_le(self):
        ...
        for links, expected in cases:
            with self.subTest(links=links):
                ...
                with (
                    patch.object(device.socket, "socket", return_value=self.socket),
                    patch.object(device.fcntl, "ioctl", side_effect=ioctl),
                ):
                    with contextlib.redirect_stderr(io.StringIO()):
                        self.assertIs(
                            await device.query(self.context()),
                            False if expected is None else expected,
                        )
    ...
    async def test_query_hci_error_prints_once_and_returns_failure(self):
        ...
        with (
            patch.object(device.socket, "socket", side_effect=error) as factory,
    ...
    async def test_connect_without_hid_only_accepts_an_existing_link(self):
        ...
        with (
            patch.object(device, "_query", side_effect=error) as query,
            contextlib.redirect_stderr(output),
        ):
            self.assertFalse(await device.connect(ctx, timeout=0.01))
        query.assert_called_once_with(ctx)
    ...
    async def test_lock_entry_failure_returns_false_once_without_touching_resources(
        self,
    ):
        for operation in (device.register_hid, device.unregister_hid, device.connect):
            with self.subTest(operation=operation.__name__):
                ctx, output = self.context(), io.StringIO()
                ctx._bus, ctx._registered, ctx._lock = (
                    self.bus,
                    True,
                    FakeLock(enter_error=OSError(5, "lock")),
                )
                with contextlib.redirect_stderr(output):
    ...
    async def test_lock_exit_failure_returns_false_once(self):
        ...
        with (
            patch.object(device, "_query", new=Mock(return_value=True)),
            contextlib.redirect_stderr(output),
        ):
    ...
    async def test_register_and_unregister_lock_exit_failures_return_false_once(self):
        ...
        ctx._lock = FakeLock(exit_error=OSError(5, "lock"))
        with contextlib.redirect_stderr(output):
            self.assertFalse(await device.unregister_hid(ctx))
        self.assertEqual(len(output.getvalue().splitlines()), 1)
        self.assertIsNone(ctx._bus)
    ...
    async def test_invalid_connect_timeout_does_not_advertise(self):
        ...
        self.assertFalse(
            any(call.member == "RegisterAdvertisement" for call in self.bus.calls)
        )

    async def test_connect_preserves_existing_hid_for_success_timeout_and_concurrency(
        self,
    ):
        ctx = self.context()
        self.assertTrue(await device.register_hid(ctx))
        ...
        with patch.object(device, "_query", new=Mock(return_value=True)):
            self.assertEqual(
                await asyncio.gather(*(device.connect(ctx, 0.01) for _ in range(3))),
                [True] * 3,
            )
    ...
    async def test_failed_close_keeps_handle_for_a_later_unregister(self):
        ...
        self.assertIs(ctx._bus, self.bus)
        self.assertEqual(
            output.getvalue(), f"错误：unregister_hid：{self.bus.wait_error}\n"
        )
    ...
    async def test_cancelled_registration_survives_a_separate_cleanup_failure(self):
        ctx = self.context()
        self.bus.connect_gate, self.bus.wait_error = (
            asyncio.Event(),
            OSError(9, "句柄无效"),
        )
        task = asyncio.create_task(device.register_hid(ctx))
        ...
        with contextlib.redirect_stderr(output):
            with self.assertRaises(asyncio.CancelledError) as raised:
                await task
        self.assertEqual(
            raised.exception.__notes__, [f"注销 HID 失败：{self.bus.wait_error}"]
        )
    ...
    async def test_cancelled_advertisement_registration_unexports_but_keeps_gatt(self):
        ...
        with patch.object(device, "_query", new=Mock(return_value=False)):
            ...
            while not any(
                call.member == "RegisterAdvertisement" for call in self.bus.calls
            ):
                await asyncio.sleep(0)
            task.cancel()
            with self.assertRaises(asyncio.CancelledError):
        ...
        with patch.object(device, "_query", new=Mock(return_value=False)):
            ...
            with self.assertRaises(asyncio.CancelledError) as raised:
                await task
        self.assertTrue(
            any("停止广播失败" in note for note in raised.exception.__notes__)
        )
        self.assertNotIn(device.ADVERTISEMENT, [path for path, _ in self.bus.exports])

    async def test_cancellation_during_advertisement_stop_unexports_and_preserves_gatt(
        self,
    ):
        for result in ("success", "timeout"):
            with self.subTest(result=result):
                ...
                with patch.object(device, "_query", new=Mock(side_effect=values)):
                    task = asyncio.create_task(device.connect(ctx, 0.01))
                    while not any(
                        call.member == "UnregisterAdvertisement"
                ...
                self.assertTrue(ctx._registered)
                self.assertNotIn(
                    device.ADVERTISEMENT, [path for path, _ in self.bus.exports]
                )

    async def test_advertisement_register_failure_logs_once_and_cleanup_ignores_missing(
        self,
    ):
        ctx, output = self.context(), io.StringIO()
        self.assertTrue(await device.register_hid(ctx))
    ...
    async def test_advertisement_cleanup_failure_returns_false_once_and_keeps_gatt(
        self,
    ):
        ctx, output = self.context(), io.StringIO()
        self.assertTrue(await device.register_hid(ctx))
        ...
        with (
            patch.object(device, "_query", new=Mock(side_effect=[False, True])),
            contextlib.redirect_stderr(output),
        ):
            self.assertFalse(await device.connect(ctx, 0.01))
        self.assertEqual(len(output.getvalue().splitlines()), 1)
    ...
    async def test_cancelled_unexport_releases_lock_and_propagates_cancellation(self):
        ...
        with patch.object(device, "_query", new=Mock(side_effect=[False, True])):
            with self.assertRaises(asyncio.CancelledError):
                await device.connect(ctx, 0.01)
        self.assertFalse(ctx._lock.locked())
        self.assertTrue(ctx._registered)

    async def test_registration_and_cleanup_failures_are_reported_together_at_boundary(
        self,
    ):
        ctx, output = self.context(), io.StringIO()
        self.bus.fail_member, self.bus.wait_error = (
            "RegisterApplication",
            OSError(9, "句柄无效"),
        )
        with contextlib.redirect_stderr(output):
        ...
        self.assertIn(
            "BlueZ RegisterApplication 失败：org.bluez.Error.Failed", lines[0]
        )
