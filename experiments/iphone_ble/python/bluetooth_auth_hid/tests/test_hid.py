...
class HidTests(unittest.TestCase):
    ...
    def test_exports_hid_hierarchy_and_object_manager(self):
        ...
        self.assertEqual(
            report["Flags"].value, ["read", "encrypt-read", "notify", "encrypt-notify"]
        )
        self.assertNotIn("Value", report)
        self.assertEqual(
            self.app.report_map.read_value(self.options), self.app.report_map.value
        )
    ...
    def test_access_requires_allowed_le_device(self):
        with self.assertRaises(DBusError) as denied:
            self.app.report.read_value(
                {
                    "device": Variant("o", "/org/bluez/hci0/dev_DENIED"),
                    "link": Variant("s", "le"),
                }
            )
        ...
        with self.assertRaises(DBusError) as malformed:
            self.app.report.read_value(
                {
                    "device": Variant("s", "/org/bluez/hci0/dev_ALLOWED"),
                    "link": Variant("s", "le"),
                }
            )
    ...
    def test_optional_services_and_export_failure_cleanup(self):
        ...
        self.assertEqual(
            {item.path for item in app.objects},
            {
                app.hid.path,
                app.hid_info.path,
                app.report_map.path,
                app.control_point.path,
                app.report.path,
                app.report_reference.path,
            },
        )
