...
async def register_hid() -> MessageBus:
    ...
    objects = [
        hid,
        _Characteristic(
            hid.path + "/char0", hid, "2a4a", b"\x11\x01\x00\x02", read, bool
        ),
        ...
        battery,
        _Characteristic(
            battery.path + "/char0", battery, "2a19", b"\x64", [*read, "notify"], bool
        ),
        device_info,
        _Characteristic(
            device_info.path + "/char0",
            device_info,
            "2a29",
            b"Bluetooth Auth",
            ["read"],
