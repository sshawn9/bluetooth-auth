...
def show(event: str, details: dict) -> None:
    print(
        json.dumps(
            {"event": event, "mode": "bluez-hid-coexist", **details},
            ensure_ascii=False,
            indent=2,
        ),
        flush=True,
...
def configured_phone(args) -> str:
    ...
    if args.phone_file is None:
        raise ValueError(
            "请用 --phone-file 指定地址文件，或设置 BLUETOOTH_AUTH_ADDRESS_FILE"
        )
    try:
...
class State:
    ...
    def lock(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            for journal in (
                old_root / "adapter-restore.json",
                old_root / "adapter-restore.json.tmp",
            ):
                if journal.exists():
                    raise RuntimeError(
                        "独占实验有待恢复记录；先执行 ble_lab.py restore"
                    )
            ...
            for path in paths:
                fd = os.open(
                    check_path(path), os.O_RDWR | os.O_CREAT | os.O_NOFOLLOW, 0o600
                )
    ...
    def emit(self, event: str, details: dict) -> None:
        fd = os.open(
            check_path(self.root / "events.jsonl"),
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as stream:
            stream.write(
                json.dumps(
                    {"time": time.time(), "event": event, **details}, ensure_ascii=False
                )
                + "\n"
            )
        show(event, details)
    ...
    def load(self) -> dict:
        path = (
            ...
            else self.journal.with_suffix(".json.tmp")
        )
        ...
        if candidate is not None and not re.fullmatch(
            r"/org/bluez/hci\d+/dev_(?:[0-9A-F]{2}_){5}[0-9A-F]{2}", candidate
        ):
...
class Backend:
    ...
    def _message(self, message):
        ...
        if (
            ...
            and (
                ...
                or (message.path or "").startswith(ROOT_PATH + "/")
            )
            ...
            return Message.new_error(
                ...
                "Only BlueZ may call this test service",
            )
        ...
        if (
            ...
            self.changes.append(
                (
                    message.path,
                    interface,
                    {name: value.value for name, value in properties.items()},
                )
            )
        elif (
            message.interface == "org.freedesktop.DBus.ObjectManager"
    ...
    async def register(self, adapter_path: str, interface: str, path: str):
        member = (
            ...
            if interface == AD_MANAGER
            else "RegisterApplication"
        )
        await self.connection.call(
            adapter_path, interface, member, "oa{sv}", [path, {}]
        )
    ...
    async def register_agent(self):
        ...
        await self.connection.call(
            "/org/bluez", AGENT_MANAGER, "RequestDefaultAgent", "o", [AGENT_PATH]
        )
    ...
    async def unregister_agent(self):
        ...
        try:
            await self.connection.call(
                "/org/bluez", AGENT_MANAGER, "UnregisterAgent", "o", [AGENT_PATH]
            )
        except BlueZCallError as error:
            if error.error_name not in {
                "org.bluez.Error.DoesNotExist",
...
def find_adapter(
    objects: dict, *, name: str | None = None, physical: str | None = None
):
    candidates = [
        ...
        and (
            ...
            if name
            else interfaces[ADAPTER].get("Address", "").upper() == physical
        )
    ]
    if len(candidates) != 1:
...
def find_phone(
    objects: dict, adapter_path: str, phone: str, *, preferred: str | None = None
):
    matches = [
        (path, interfaces[DEVICE])
        for path, interfaces in objects.items()
        if DEVICE in interfaces
        and interfaces[DEVICE].get("Adapter") == adapter_path
    ...
    if len(matches) != 1:
        raise RuntimeError(
            "BlueZ 中没有唯一的目标手机记录；不会扫描、创建配对或连接其他手机"
        )
    return matches[0]
...
def links_json(links):
    return [
        {
            "address": peer,
            "transport": "bredr" if kind == 0 else "le",
            "address_type": kind,
        }
...
async def restore(state: State, *, backend_factory=Backend, link_factory=None) -> dict:
    ...
    link = None
    try:
        if backend.bus_id == journal["bus_id"] and await backend.owner_alive(
            journal["owner"]
        ):
            raise RuntimeError(
                "原共存测试的 D-Bus 连接仍在；先退出该进程，保留恢复记录"
            )
        ...
        while True:
            ...
            added_target = {
                (peer, kind)
                for peer, kind in current - baseline
                if peer in cleanup_peers and kind in (1, 2)
            }
            hid_present = HID_UUID in {uuid.lower() for uuid in props.get("UUIDs", [])}
            active_advertisements = (
                objects.get(path, {}).get(AD_MANAGER, {}).get("ActiveInstances", 0)
            )
            ads_released = active_advertisements <= journal.get(
            ...
            if (
                ...
                and not hid_present
                and ads_released
                and props.get("Pairable") == journal["properties"]["Pairable"]
            ):
                break
            if asyncio.get_running_loop().time() >= deadline:
                raise RuntimeError(
                    "目标新增 LE 连接、HID 服务、广播计数或 Pairable 尚未恢复；保留恢复记录"
                )
            await asyncio.sleep(POLL_SECONDS)
        changed = [
            key
            for key, value in journal["properties"].items()
            if props.get(key) != value
        ]
        try:
            _, phone = find_phone(
                objects, path, journal["phone"], preferred=journal.get("phone_path")
            )
            pairing_changed = [
                key
                for key, value in journal["phone_properties"].items()
                if phone.get(key) != value
            ]
        except RuntimeError:
            pairing_changed = ["device_record_missing"]
        disallowed_pairing_changes = [
            key
            for key in pairing_changed
            if not repair_pairing or key not in {"Paired", "Bonded"}
        ]
        if changed or disallowed_pairing_changes:
            raise RuntimeError(
                f"恢复核对未通过：adapter={changed}, pairing={pairing_changed}；未强行改写配对记录，保留记录"
            )
        missing = baseline - current
        added_classic = {
            (peer, kind)
            for peer, kind in current - baseline
            if peer in cleanup_peers and kind == 0
        }
        ...
        result = {
            ...
            "alias": props["Alias"],
            "test_bus_released": True,
            "new_phone_le_disconnected": links_json(removed),
            "test_hid_service_removed": True,
            "advertising_instances": active_advertisements,
            "original_connections_present": not missing,
            "connections_restored": not missing and not added_classic,
            "connections_needing_check": links_json(missing),
            "pairing_properties_unchanged": not pairing_changed,
            "new_phone_classic_connections": links_json(added_classic),
            "original_links_interrupted": links_json(
                {tuple(item) for item in journal.get("original_links_interrupted", [])}
            ),
        }
        if repair_pairing:
            result.update(
                pairing_records="retained",
                pairing_keys_restored=False,
                test_pairing_agent_removed=True,
                message="广播、HID 和配对代理已撤销，适配器设置已恢复；本轮 BlueZ 配对记录保留",
            )
        if added_classic:
            result["message"] = (
                "存在本轮新增的手机经典蓝牙连接；未擅自断开，恢复记录保留"
            )
...
async def run(
    ...
    *,
    ...
    def phone_disconnect_count():
        return sum(
            event["address"] in ({phone_address} | pairing_addresses)
            and event["address_type"] in (1, 2)
    ...
    def allow_device(device_path):
        ...
        if repair_pairing:
            ...
            return bool(
                ...
                and agent.accepted
                and (
                    device_path == agent.confirmed_device
                    or info.get("Address", "").upper() == phone_address
                )
            )
        return info.get("Address", "").upper() == phone_address and info.get(
            "Paired", False
        )
    ...
    async def allow_pairing(device_path):
        ...
        if peer != phone_address and (
            ...
            if device_path != candidate_path:
                return False
        session_peers = {peer} | (
            pairing_addresses if device_path == candidate_path else set()
        )
        current = await link.connections()
    ...
    def emit(event, payload):
        nonlocal \
            unexpected_release, \
            access_disconnect_count, \
            report_subscription_seen, \
            candidate_path
        ...
        if event == "pairing_confirmation_accepted":
            ...
            state.save(journal)
        if (
            event == "target_gatt_access"
            and allow_device(payload.get("device"))
            and payload.get("link", "").lower() == "le"
        ):
            hid_access.set()
        ...
        if "device" in payload:
            payload["device"] = (
                "target_phone"
                if allow_device(payload["device"])
                else "pairing_candidate"
                if payload["device"] == candidate_path
                else "other_device"
            )
    ...
    try:
        ...
        if props["Pairable"] and props["PairableTimeout"]:
            raise RuntimeError(
                "适配器正处于限时配对窗口，无法恢复其剩余计时；请待窗口结束后测试"
            )
        target_path, phone = find_phone(objects, path, phone_address)
        if phone.get("Blocked"):
            raise RuntimeError("目标手机被阻止；不会为实验解除阻止")
        if not repair_pairing and (
            not phone.get("Paired") or not phone.get("Bonded", phone.get("Paired"))
        ):
            raise RuntimeError(
                "目标手机必须已有可用的 BlueZ 配对；不会重新配对或解除阻止"
            )
        ...
        if not companions:
            raise RuntimeError(
                "共存测试需先让电脑连着一台已有蓝牙耳机或鼠标；没有注册服务或广播"
            )
        baseline_audio = {
            object_path
        ...
        journal = {
            ...
            "phone": phone_address,
            "phone_properties": {
                key: phone[key] for key in PHONE_PROPS if key in phone
            },
            "phone_path": target_path,
            "repair_phone_pairing": repair_pairing,
            "pairing_addresses": [],
            "pairing_confirmation_accepted": False,
            "baseline_links": sorted(baseline),
            "owner": backend.bus.unique_name,
            "original_links_interrupted": [],
            "advertising_instances": objects[path][AD_MANAGER].get(
                "ActiveInstances", 0
            ),
            "bus_id": backend.bus_id,
            "bluez_owner": backend.bluez_owner,
        }
        state.save(journal)
        phase = "register"
            ...
            if confirm_pairing is None:
                ...
                async def confirm_pairing(number):
                    return await _terminal_confirm_pairing(
                        number, 6, args.wait_seconds, emit
                    )
        ...
        emit(
            "advertising",
            {
                "adapter": args.adapter,
                "name": props["Alias"],
                "expected_name_location": "scan_response",
                "controller_trace_enabled": trace is not None,
                "shared_bluez_identity": True,
                "companion_links": links_json(companions),
                "new_pairing_allowed": repair_pairing,
                "wait_seconds": args.wait_seconds,
                "phone_side_pairing_confirmed": False,
                "message": (
                    f"在 iPhone 蓝牙设置选择 {props['Alias']}，核对手机和终端的数字后确认；新配对保留。"
                    if repair_pairing
                    else f"前提：手机和电脑均保留原配对；在 iPhone 选择 {props['Alias']}，手机已忽略配对则使用 --repair-phone-pairing。"
                ),
            },
        )
        ...
        while True:
            ...
            for object_path, interface, change in backend.changes:
                if interface == DEVICE and (
                    change.get("Connected") is False or change.get("removed")
                ):
                    info = objects.get(object_path, {}).get(DEVICE, {})
                    peer = info.get(
                        "Address", object_path.rsplit("/dev_", 1)[-1].replace("_", ":")
                    ).upper()
                    lost_links |= {
                        candidate for candidate in baseline if candidate[0] == peer
                    }
            if lost_links:
            ...
            if (
                current_props.get("Powered") is not True
                or current_props.get("Pairable") != repair_pairing
            ):
                raise RuntimeError("适配器电源或配对许可被改变，终止测试")
            target_path, phone_now = find_phone(
                objects, path, phone_address, preferred=candidate_path
            )
            watched = {"Trusted", "Blocked"} if repair_pairing else set(PHONE_PROPS)
            if any(
                phone_now.get(key) != value
                for key, value in journal["phone_properties"].items()
                if key in watched
            ):
            ...
            if repair_pairing:
                ...
                identity_verified = bool(
                    agent.accepted
                    and candidate.get("Address", "").upper() == phone_address
                    and candidate.get("Paired")
                    and candidate.get("Bonded", candidate.get("Paired"))
                )
            session_peers = {phone_address} | (
                pairing_addresses if identity_verified else set()
            )
            phone_le = {
                (peer, kind)
                for peer, kind in current
                if peer in session_peers and kind in (1, 2)
            }
            phone_paths = {
                object_path
                for object_path, interfaces in objects.items()
                if interfaces.get(DEVICE, {}).get("Adapter") == path
                and (
                    interfaces[DEVICE].get("Address", "").upper() == phone_address
                    or object_path == candidate_path
                )
            }
            audio = {
                object_path
            ...
            if (
                ...
                and hid_access.is_set()
                and phone_le
                and access_disconnect_count == phone_disconnect_count()
            ):
                ready_at = now
                ...
                phase = "hold"
                result.update(
                    encrypted_phone_hid_access=True, companion_count=len(companions)
                )
                emit(
                    "service_ready",
                    {
                        "encrypted_phone_hid_access": True,
                        "report_subscription_scope": "global",
                        "hold_seconds": args.hold_seconds,
                        "message": "现在同时使用原耳机/鼠标，检查是否卡顿或断开，并检查手机音频输出。",
                    },
                )
            if ready_at is not None and (
                not phone_le or phone_disconnect_count() != ready_disconnect_count
            ):
                ...
                if ready_at is None:
                    if repair_pairing and agent.accepted and not identity_verified:
                        raise ObservationIncomplete(
                            "已确认配对数字，但尚未确认候选与配置的手机身份一致且已保存配对；不判共存通过"
                        )
                    if repair_pairing and not agent.accepted and phone_le:
                        raise ObservationIncomplete(
                            "目标 LE 链路存在，但本轮没有完成数字核对；不能把电脑旧 Paired 状态当作重新配对成功"
                        )
                    if phone_le:
                        raise ObservationIncomplete(
                            "目标 LE 链路存在，但未观察到加密 HID 读取；手机可能使用缓存，当前证据不足以判断服务可用"
                        )
                    raise TimeoutError(
                        "等待期内未观察到目标手机 LE 连接；现有观测不能区分广播未被发现、手机筛选、配对记录不一致等原因"
                    )
                result.update(
                    passed=True,
                    hold=True,
                    original_links_preserved=True,
                    hold_seconds=args.hold_seconds,
                    new_phone_audio_transports=0,
                )
            ...
            if now >= next_progress:
                emit(
                    "waiting" if ready_at is None else "holding",
                    {
                        "remaining_seconds": round(max(0, deadline - now), 1),
                        "phone_le_connected": bool(phone_le),
                        "encrypted_hid_read_seen": hid_access.is_set(),
        ...
        phase = "cleanup"
        failures = []
        for attempted, interface, object_path in (
            (attempted_ad, AD_MANAGER, AD_PATH),
        ...
        if agent is not None:
            ...
            result.update(
                pairing_confirmation_accepted=agent.accepted,
                pairing_identity_verified=identity_verified,
                pairing_records="retained",
            )
        ...
        try:
            restoration = await asyncio.shield(
                restore(
                    state, backend_factory=backend_factory, link_factory=link_factory
                )
            )
            emit("restore", restoration)
            if not restoration.get(
                "connections_restored",
                restoration.get("original_connections_present", True),
            ):
                result["passed"] = False
                status = 1
        except Exception as error:
            ...
            emit(
                ...
                {
                    "reason": str(error)[:350],
                    "message": "保留恢复记录，请运行 bluez_hid_lab.py restore",
                },
            )
        if failures:
...
def parser():
    ...
    phone.add_argument(
        "--phone-file",
        type=Path,
        default=os.environ.get("BLUETOOTH_AUTH_ADDRESS_FILE") or None,
    ...
    try:
        if args.command in (None, "plan"):
            show(
                "plan",
                {
                    "adapter": "明确指定现有 hciN，不关闭或独占控制器",
                    "name": "沿用原 BlueZ Alias",
        ...
        if os.geteuid() != 0:
            raise PermissionError(
                "run/有遗留记录的 restore 需要 sudo；脚本不会自行提权"
            )
        if args.command == "run":
            if not re.fullmatch(r"hci\d+", args.adapter):
                raise ValueError("--adapter 必须是明确的 hciN")
            if not all(
                0 < value <= 180 for value in (args.wait_seconds, args.hold_seconds)
            ):
        ...
        with state.lock():
            ...
            return (
                ...
                if result.get(
                    "connections_restored",
                    result.get("original_connections_present", True),
                )
                else 1
            )
