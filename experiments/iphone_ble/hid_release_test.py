...
def uuids(adapter):
    return {
        value.lower().split("-", 1)[0].lstrip("0") for value in adapter.get("UUIDs", [])
    }
...
def connected_links(reader, index, phone):
    return [
        link
        for link in reader.read(index)
        if link.address == phone
...
def hid_ready(state):
    return (
        state.get("gatt_registered") is True
        and state.get("advertisement_registered") is True
        and state.get("hid_uuid_present") is True
        and state.get("advertising_instances") == 1
    )
...
def connection_ready(state):
    ...
    return (
        hid_ready(state)
        and state.get("phone_le_connected") is True
        and state.get("hci_le_links") == 1
        and state.get("hci_encrypted") is True
    )
...
async def stop_provider(child):
    ...
    try:
        ...
        await child.wait()
        raise Inconclusive(
            f"HID 子进程停止超时，已终止；退出码 {child.returncode}，本轮不能判通过"
        )
...
async def experiment(args, phone):
    ...
    connection_setup = (
        "manual" if getattr(args, "manual_connect", False) else "automatic"
    )
    ...
    try:
        ...
        show(
            "initial_resources",
            hid_uuid_present="1812" in uuids(adapter),
            advertising_instances=objects.get(adapter_path, {})
            .get(AD_MANAGER, {})
            .get("ActiveInstances"),
        )
        ...
        if adapter.get("Powered") is not True or GATT_MANAGER not in objects.get(
            adapter_path, {}
        ):
        ...
        if (
            "1812" in uuids(adapter)
            or objects[adapter_path][AD_MANAGER].get("ActiveInstances", 0) != 0
        ):
            raise Inconclusive("请先退出其他 HID 程序及广播实验，再运行本短测")
        original_links = await mgmt.connections()
        existing_target = {
            (peer, kind)
            for peer, kind in original_links
            if peer == phone and kind in (1, 2)
        }
        ...
        async def snapshot():
            ...
            if current_flags != saved_phone:
                show(
                    ...
                    changes={
                        key: {"before": saved_phone[key], "now": current_flags[key]}
                        for key in PHONE_PROPS
                        if current_flags[key] != saved_phone[key]
                    },
                )
            ...
            lost = {
                ...
                for item in mgmt.disconnect_events
            }
            ...
            if phase in {"register", "connect"}:
                ...
                if initial_handle is not None and (
                    len(links) != 1 or links[0].handle != initial_handle
                ):
                    raise LinkLost("准备期间原目标 LE 消失或连接句柄已改变")
            state.update(
                gatt_registered=signals["gatt_registered"],
                advertisement_registered=signals["registered"],
                hid_uuid_present="1812" in uuids(current_adapter),
                advertising_instances=objects[adapter_path][AD_MANAGER].get(
                    "ActiveInstances", 0
                ),
                ...
                hci_encrypted=len(links) == 1 and links[0].encrypted,
        ...
        child = await asyncio.create_subprocess_exec(
            ...
            "--provider",
            "--phone-file",
            str(Path(args.phone_file).resolve()),
            "--adapter",
            args.adapter,
        ...
        async def read_events():
            async for line in child.stdout:
                ...
                if item["event"] == "gatt_registered":
                    ...
                    show(
                        "provider_error",
                        phase=item.get("phase", "entry"),
                        error_type=item.get("error_type", "Unknown"),
                        dbus_error=item.get("dbus_error"),
        ...
        def check_provider():
            ...
            if child.returncode is not None:
                ...
                raise Inconclusive(
                    f"HID 提供进程提前退出，退出码 {child.returncode}；没有进入注销后观察阶段"
                )
            if output_task.done():
        ...
        show(
            "advertising",
            hid_uuid=HID_UUID,
            appearance=f"0x{HID_APPEARANCE:04x}",
            interval_ms=ADVERTISING_INTERVAL_MS,
            shared_bluez_identity=True,
            name_source="adapter_alias",
            **state,
        )
        phase = "connect"
        show(
            "checking_existing_connection"
            if connection_setup == "existing"
            else "waiting_for_connection",
            timeout_seconds=args.wait_seconds,
            connection_setup=connection_setup,
            message=(
                "HID 服务和广播已注册；正在核对已有 LE 的加密状态，请保持手机锁屏。"
                if connection_setup == "existing"
                else "HID 服务和广播已注册；在手机点击电脑原配对条目连接，连上后立即锁屏。"
                if connection_setup == "manual"
                else "HID 服务和广播已注册；手机保持锁屏，等待目标加密 LE 连接。"
            ),
        )
        ...
        show(
            ...
            baseline_seconds=5,
            connection_setup=connection_setup,
            message="请保持手机锁屏；5 秒后退出 HID 提供进程。",
            **state,
        )
        ...
        await output_task
        state.update(
            provider_returncode=child.returncode, provider_dbus_closed=signals["closed"]
        )
        show("provider_exit", **state)
        if child.returncode != 0 or signals["error"] or not signals["closed"]:
            raise Inconclusive(
                "HID 子进程退出或 D-Bus 关闭未通过，见 provider_exit/provider_error"
            )
        async with asyncio.timeout(5):
            while True:
                ...
                if (
                    not owner_alive
                    and "1812" not in uuids(objects[adapter_path][ADAPTER])
        ...
        deadline, next_progress = (
            observe_started + args.observe_seconds,
            observe_started,
        )
        while True:
            objects, links = await snapshot()
            if (
                "1812" in uuids(objects[adapter_path][ADAPTER])
            ...
            if now >= next_progress:
                show(
                    "observing",
                    elapsed_seconds=round(now - observe_started, 1),
                    same_le_link=True,
                    encrypted=True,
                )
        ...
        show(
            "result",
            verdict="failed" if negative else "inconclusive",
            phase=phase,
            reason=str(error),
            connection_setup=connection_setup,
            observed_seconds=round(time.monotonic() - observe_started, 1)
            if observe_started
            else 0,
        )
        ...
        show(
            ...
            verdict="inconclusive",
            phase=phase,
            reason=timeout_reason(phase, state),
            state=state,
            connection_setup=connection_setup,
        )
        return 2
...
def main():
    ...
    parser.add_argument(
        "--phone-file", required=True, help="已有私有地址文件，仅含手机身份地址一行"
    )
    ...
    try:
        ...
        return (
            asyncio.run(
                provider(args, phone) if args.provider else experiment(args, phone)
            )
            or 0
        )
