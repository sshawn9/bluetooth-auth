...
本目录保存使用 iPhone 系统功能比较 ANCS、消费控制类 HID over GATT、CTS 的实验代码、结果和复现材料，以及后续接入认证后的实机故障排查。

**2026-09-15 双模 LE 恢复专题：** 从[背景、全部过程与场景结论](results/LE_BEARER_RECOVERY.zh-CN.md)开始阅读，配套[复核工具与操作手册](docs/le-bearer-recovery/REPRODUCE.md)、[源码资料](docs/le-bearer-recovery/REFERENCES.md)及 651 条留存事件。它补充了经典连接与 LE 恢复策略的关系，也记录了仍未查明的原始断线起点。
...
上表的早期连接实验采用**电脑提供广播，由 iPhone 连回**；BZ-HID-05 复用既有连接，BZ-HID-07 从未连接状态开始，两者均通过服务撤销后的短时保持。按需临时提供 HID 有实测依据；结论不覆盖长期稳定性、所有恢复场景、所有音频服务隔离或正式认证安全。BZ-HID-06/07 的 Trusted 条件也发生变化，不能将两轮对比当作 HID 是唯一恢复原因的证明。9 月 15 日专题另有电脑主动建立 LE 的对照，不应反推到这些旧实验。
