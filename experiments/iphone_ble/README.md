...
这里的重连方式是**电脑恢复广播，由 iPhone 连回**。结论只覆盖本次环境和短测，不表示长期稳定性、所有音频服务隔离或正式认证安全已经验证。

## 阅读顺序

| 文档或材料 | 用途 |
| --- | --- |
| [实验报告 RESULTS.md](RESULTS.md) | 明确结论、13 项成功/失败/恢复记录、证据等级、失误经验及未覆盖范围 |
| [复现手册](docs/REPRODUCE.md) | 环境准备、各场景前提、可直接执行的命令、手机操作、成功判据和恢复方法 |
| [实现说明](docs/IMPLEMENTATION.md) | 文件职责、控制器/身份/名称、BLE 与 GATT 角色、两个入口的代码与恢复边界 |
| [运行记录模板](docs/RUN_RECORD_TEMPLATE.md) | 后续复现时记录环境、条件、原始输出、人工观察和恢复结果 |
| [证据目录说明](results/README.md) | 完整脱敏附件、对话摘录、机器可读记录与校验清单的来源说明 |

`run` 会访问真实蓝牙，独占模式会中断原有连接。请按复现手册选择场景并核对恢复结果；离线回归不能替代实机验证。
