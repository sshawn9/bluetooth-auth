...
class BlueZBackend:
    ...
    async def call(
        self, path, interface, member, signature="", body=None, destination="org.bluez"
    ):
    ...
    async def objects(self):
        ...
        return {
            path: {
                interface: {key: value.value for key, value in props.items()}
    ...
    async def set(self, path, name, value):
        ...
        try:
            ...
            raise BlueZCallError(
                error.error_name, f"Set({name}={value!r})", error.details
            ) from error
...
async def acquire(adapter: str, journal_path: Path, *, backend=None) -> dict:
    ...
    if (
        journal_path.exists()
        or journal_path.with_name(journal_path.name + ".tmp").exists()
    ):
    ...
    try:
        ...
        if not props["Powered"] and (
            props["Discoverable"] or props.get("Connectable", False)
        ):
            raise RuntimeError(
                "已关闭的适配器仍标记可发现/可连接；请等待 BlueZ 状态稳定后重试，没有修改设置"
            )
        ...
        connected = [
            info[DEVICE].get("Address")
            for info in objects.values()
            if DEVICE in info
            and info[DEVICE].get("Adapter") == path
            and info[DEVICE].get("Connected")
        ]
        journal = {
            "kind": KIND + "/adapter",
            "phase": "prepared",
...
def _read_journal(path: Path) -> dict:
    ...
    if journal.get("kind") != KIND + "/adapter" or journal.get("address") != props.get(
        "Address"
    ):
        raise ValueError("恢复记录不合法，拒绝修改适配器")
    if not re.fullmatch(
        r"(?:[0-9A-Fa-f]{2}:){5}[0-9A-Fa-f]{2}", str(journal.get("address", ""))
    ):
...
async def _restore_property(backend, address, name, value, deadline, emit):
    ...
    while True:
        ...
        matches = [
            ...
            and interfaces[ADAPTER].get("Address", "").upper() == address.upper()
        ]
        if len(matches) > 1:
        ...
        if remaining <= 0:
            raise RuntimeError(
                f"恢复 {name}={value!r} 超时：{last_reason}；恢复记录保留"
            )
...
async def restore(
    journal_path: Path, *, backend=None, timeout: float = 15, emit=None
) -> dict:
    ...
    try:
        ...
        while True:
            ...
            matches = [
                ...
                for path, interfaces in objects.items()
                if ADAPTER in interfaces
                and interfaces[ADAPTER].get("Address", "").upper()
                == journal["address"].upper()
            ]
            if len(matches) == 1:
                path, current = matches[0]
                break
            if len(matches) > 1 or asyncio.get_running_loop().time() >= end:
                raise RuntimeError(
                    "未能按原物理地址唯一找到适配器；保留恢复记录，请释放实验进程后重试 restore"
                )
        ...
        order.extend(
            name for name in ("Connectable", "Discoverable") if name in previous
        )
        for name in order:
            await _restore_property(
                backend, journal["address"], name, previous[name], end, emit
            )
        # Verify the postcondition instead of trusting successful Set replies.
        objects = await backend.objects()
        matches = [
            (candidate_path, interfaces[ADAPTER])
            for candidate_path, interfaces in objects.items()
            if ADAPTER in interfaces
            and interfaces[ADAPTER].get("Address", "").upper()
            == journal["address"].upper()
        ]
        ...
        missing = [
            address
            for address in journal["previously_connected"]
            if address not in now_connected
        ]
        result = {
            "settings_restored": True,
            "changed": True,
            "adapter": path.rsplit("/", 1)[-1],
            "connections_needing_check": missing,
